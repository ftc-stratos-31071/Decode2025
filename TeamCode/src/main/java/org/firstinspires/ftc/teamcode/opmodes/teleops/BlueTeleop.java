...
public class BlueTeleop extends NextFTCOpMode {
    ...
    public void onStartButtonPressed() {
        ...
        Command driverControlled = new MecanumDriverControlled(
                ...
                backLeftMotor,
                backRightMotor,
                Gamepads.gamepad1().leftStickY().map(v -> v * DRIVE_SCALE),
                Gamepads.gamepad1().leftStickX().negate().map(v -> v * DRIVE_SCALE),
                Gamepads.gamepad1().rightStickX().negate().map(v -> v * DRIVE_SCALE)
        );
